...
@MainActor
final class SegmentViewModel: ObservableObject, Identifiable {
    ...
    var departureDateText: String? {
        ...
        return SegmentViewModel.shortDateFormatter.string(from: dep)
    }
    
    var departureTimeText: String? {
        guard let dep = segment.departure else { return nil }
        return SegmentViewModel.timeFormatter.string(from: dep)
    }
    
    var arrivalTimeText: String? {
        guard let arr = segment.arrival else { return nil }
        return SegmentViewModel.timeFormatter.string(from: arr)
    }
